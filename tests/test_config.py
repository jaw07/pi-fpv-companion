...
def test_loads_imx500_yaml():
    ...
    assert cfg.camera.type == "imx500"        # flight camera (on-sensor inference)
    assert cfg.detector.type == "none"        # IMX500 emits detections inline — no CPU detector
    assert cfg.tracker.type == "multi_iou"
...
def test_unknown_keys_dont_crash(tmp_path):
    """Future-proofing — adding a new key shouldn't break old loaders."""
